/*function mostrarFormularioEdicao(item) {
    ...
    <label>Nome e Sobrenome: <span>${item.nome_sobrenome}</span></label><br>
...
document.addEventListener('DOMContentLoaded', function() {
    const visualizarApagarLink = document.querySelector('a[href="#"][onclick="showPage(\'visualizarapagar\')"]');
    if (visualizarApagarLink) {
        visualizarApagarLink.addEventListener('click', function(event) {
...
// Supondo que a variável cpf já esteja definida em algum lugar do seu código
var chamarcpf = "";
...
document.addEventListener('DOMContentLoaded', function() {
    const linkAlterarDados = document.querySelector('a[href="#"][onclick="showPage(\'visualizarapagar\')"]');
    if (linkAlterarDados) {
        linkAlterarDados.addEventListener('click', function(event) {
            event.preventDefault();
            const cpf = document.getElementById('cpf').value.trim();
...
// Chama a função buscarDadosPorCpf ao carregar a página
...
function updateUserInterface(user) {
    ...
    userName.innerText = `Nome: ${user.nome_usuario}`;
...
$(document).ready(function() {
    ...
    // Função para exibir as imagens do array imagemarray
    function exibirImagens(imagens) {
        let imagensHtml = '';
        imagens.forEach(imagem => {
            imagensHtml += `<img src="${imagem}" alt="Imagem do produto">`;
        });
        $('#detalhes').append(imagensHtml);
    }
        ...
        success: function(response) {
            ...
            if (response && response.length > 0) {
                ...
                $(document).on('click', 'h2', function() {
                    ...
                    // Exibir os detalhes do produto correspondente e ocultar a lista de produtos
                    $('#detalhes').html(formatarDetalhes(produtoClicado));
                    exibirImagens(produtoClicado.imagemarray);
                    mostrarSection('visualizarapagar2');
                });
            } else {
                $('#produtos').html('<p>Nenhum produto encontrado.</p>');
